...
func (app *App) ChatHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost {
		...
		if err != nil {
			app.ErrorLog.Println("error inserting prompo into database", err)
		}
		...
		// app.InfoLog.Println("USER ID ", userID)

		// conversationID := uuid.New()
		// var timestamp = time.Now().Unix()
		// prompt.UserID = userID
		// prompt.Timestamp = timestamp
		// prompt.ConversationID = conversationID.String()
		...
		json.NewEncoder(w).Encode(models.GeneratedText{
			UserID:         "Gemini_AI",
			ConversationID: prompt.ConversationID, // Use conversation of the prompt created message or chat
			Text:           generatedText,
...
// 		ChatDB := make(map[string]models.GeneratedText)
...
// 		// }
// 		// generatedText, err := text.GenTextContent(ctx, client, prompt.Text)
// 		// if err != nil {
...
// 		ChatDB[chatID.String()] = models.GeneratedText{
// 			ChatID:    chatID.String(),
// 			Text:      generatedText,
// 			Timestamp: timestamp,
// 			Metadata: models.Metadata{
